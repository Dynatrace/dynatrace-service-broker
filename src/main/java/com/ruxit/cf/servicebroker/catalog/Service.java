...
final class Service {
    ...
    private final static String DEFAULT_UUID = "99028E5B-5F0E-47CB-A8F9-B9F504505CB8"; // change for the next release
    ...
    Service addAllPlans(/*@Value("${RUXIT_PLANS}")*/ Map<String,RuxitPlan> ruxitPlans) throws ParseException {

        for (RuxitPlan ruxit_plan: ruxitPlans.values()) {
            String ruxitPlanId = ruxit_plan.getPlanId();
            String ruxitPlanName = ruxit_plan.getName();

            plan()
                .id(UUID.fromString(ruxitPlanId))
                .name(ruxitPlanName)
                .description("Service Plan for " + ruxitPlanName)
                .metadata()
                    ...
                    .displayName(ruxitPlanName.toUpperCase())
                    .cost()
                        .amount("usd", 0.0)
                        .unit("MONTHLY")
                        .and()
